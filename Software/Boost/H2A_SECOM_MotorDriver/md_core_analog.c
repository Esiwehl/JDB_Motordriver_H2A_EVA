...
#define CC_TURBO_BOOST		1								//1 beter dan 7
...
typedef struct {
	...
	uint8_t booststate;
	uint8_t idealDiodeState, boostState;
	uint32_t idealDiodeTimestamp, boostTimestamp;
...
static void InitCoreAnalogCalibration(void) {
	...
	TRYREADEEPROM(sCal.scVoltageScale, EE_SC_VSCALE);				//aangepast was fc
	TRYREADEEPROM(sCal.scCurrentScale, EE_SC_ISCALE);				//aangepast was fc
...
if(sSensorData.adc.h2a.booststate != selRegenPin) {
	 ...
	 sSensorData.adc.h2a.boostState = selRegenPin;
	 sSensorData.adc.h2a.boostTimestamp = sSessionCycleCount;
}
